...
|--------|------|------------|--------------|--------|
| **N-gram** | Statistical (7-gram Stupid Backoff) | 25,673 vocabulary, 4-7 gram tables | No | Primary, always available |
| **Transformer** | Neural (decoder-only GPT) | ~13.8M trainable parameters | Yes (training only) | Alternative, trained on Colab |
...
|-----------|-------|-------------|
| `vocab_size` | 8,000 | BPE vocabulary (SentencePiece) |
...
| **Total parameters** | **~13.8M** | Trainable parameters |
...
```bash
python tokenizer/build_tokenizer.py [--vocab_size 8000] [--input story.txt]
```
...
├── evaluate.py                         # Evaluation framework (perplexity, top-k, latency, memory)
...
├── evaluation_results.json             # Latest evaluation run output
...
│
├── tokenizer/                          # SentencePiece BPE tokenizer (vocab = 8,000)
│   ├── __init__.py
...
A standalone evaluation framework that measures both engines on held-out test data
(text after the N-gram training cutoff of 10M characters in `story.txt`).

### Evaluation Script

```bash
python evaluate.py                                  # full evaluation (~5 min)
python evaluate.py --skip-latency --skip-memory     # quality metrics only (~2 min)
python evaluate.py --test-chars 100000              # larger test set
```
...
|------|---------|-------------|
| `--test-chars` | 50,000 | Characters to use from held-out test split |
| `--latency-repeats` | 5 | Generation runs per prompt (first is warm-up) |
| `--latency-prompts` | 10 | Number of test prompts for latency measurement |
| `--skip-latency` | off | Skip latency benchmarks |
| `--skip-memory` | off | Skip memory profiling (requires fresh model load) |
| `--output` | `evaluation_results.json` | JSON output path |
| `--seed` | 42 | Random seed |

**Metrics computed:**
- **Perplexity** — geometric mean token probability on held-out tokens (lower is better)
- **Bits-per-character (BPC)** — cross-entropy in bits per raw character
- **Top-k accuracy** — fraction of positions where the true next token is in the top-1/3/5 predictions
- **Latency** — mean, median, P95, and tokens/second for sentence generation
- **Memory** — model load peak RAM and inference peak RAM via `tracemalloc`

Results are saved to `evaluation_results.json` and printed as markdown tables.

### Results

Evaluated on 50,000 characters held out from `story.txt` (chars 10M–10.05M),
measured on CPU, 2026-06-18.

#### Table 1 — Model Overview

| Property | N-gram (7-gram) | Transformer (GPT) |
|---|---|---|
| Architecture | Stupid Backoff 7→1 | 6-layer decoder-only |
| Vocabulary | 25,673 (whitespace) | 8,000 (BPE) |
| Parameters | N/A (count tables) | 13.8M |
| Training tokens | 1,886,390 | trained on Colab |
| Disk size | 247.5 MB | 158.1 MB |

#### Table 2 — Language Modeling Quality

| Metric | N-gram | Transformer |
|---|---|---|
| Perplexity | **76.62** | 335.77 |
| Bits-per-character | **1.1711** | 1.8409 |
| Top-1 Accuracy | **57.65%** | 22.71% |
| Top-3 Accuracy | **63.73%** | 35.57% |
| Top-5 Accuracy | **66.37%** | 41.64% |
| OOV Rate | 12.74% | N/A (BPE) |
| Eval tokens | 9,354 | 10,969 |

#### Table 3 — Generation Performance

| Metric | N-gram | Transformer |
|---|---|---|
| Mean latency (ms) | **5.2** | 447.4 |
| Median latency (ms) | **4.6** | 446.9 |
| P95 latency (ms) | **7.3** | 468.2 |
| Tokens/second | **9,527.7** | 111.3 |

#### Table 4 — Resource Usage

| Metric | N-gram | Transformer |
|---|---|---|
| Model load peak RAM (MB) | 3,863.3 | **53.1** |
| Inference peak RAM (MB) | 0.0 | 0.0 |
| Disk footprint (MB) | 247.5 | **158.1** |
| Parameter memory (MB) | N/A | 52.7 |

#### Table 5 — N-gram Backoff Level Distribution

| Backoff Level | Frequency |
|---|---|
| 7-gram | 43.7% (4,088) |
| 6-gram | 6.9% (647) |
| 5-gram | 7.8% (727) |
...
The N-gram engine outperforms the Transformer on every quality and speed metric on this
corpus. The Transformer's higher perplexity reflects the smaller training run (Colab GPU,
limited steps) and the domain mismatch between the English WikiText corpus and the model's
BPE vocabulary. The N-gram's advantage in backoff coverage (43.7% at 7-gram level) shows
the training data is dense enough to provide high-order context matches most of the time.
